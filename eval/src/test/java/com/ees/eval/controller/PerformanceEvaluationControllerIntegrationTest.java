...
/**
 * PerformanceEvaluationController의 실물 통합 테스트 클래스입니다.
 * 실제 MSSQL 컨테이너 환경에서 Thymeleaf 템플릿 엔진 렌더링 결과와 HTML 마크업의 회귀 안정성을 검증합니다.
 */
@SpringBootTest
@Transactional
@Timeout(value = 60, unit = TimeUnit.SECONDS)
class PerformanceEvaluationControllerIntegrationTest extends AbstractMssqlTest {
    ...
    static class TestConfig {
        ...
        public Executor testVirtualThreadExecutor() {
            // 호출 스레드에서 즉시 실행하여 동시성 문제를 방지합니다.
            return Runnable::run;
        }
    }
    ...
    @Test
    @DisplayName("통합 테스트 - 평가 차수가 PLANNED 상태일 때, 실물 HTML 내부에 검색 필터 Form과 준비 중 배너가 동시에 완벽하게 공존하여 노출되는지 검증한다")
    ...
    void should_renderFilterAndBannerTogether_when_periodIsPlanned_Integration() throws Exception {
        // 1. Given - 데이터베이스에 PLANNED 차수 직접 영속화
        EvaluationPeriod period = EvaluationPeriod.builder()
                .periodYear(2027)
                .periodName("2027년 상반기 평가")
                .statusCode("PLANNED")
        ...
        // 2. When & Then - 실제 요청 시 HTML 렌더링 마크업 검사
        mockMvc.perform(get("/eval/performance")
                ...
                // departments 부서 기본 드롭다운 정보가 전달되었는지 확인
                .andExpect(model().attributeExists("departments"))
                // 3. HTML 본문 내 id="filterForm" 과 "평가 준비 중" 텍스트의 동시 공존 강력 검증
